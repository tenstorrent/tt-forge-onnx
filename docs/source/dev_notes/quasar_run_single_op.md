...
For what Quasar is and how the architecture reaches the compiler, see
`forge/test/mlir/test_target_arch.py`, which compiles for it with no device
attached. This page is only the mechanics of running.
...
> step 3 tells you which to run; the module docstring of
> `forge/test/mlir/test_quasar_sim.py` explains why the two differ.
...
the module docstring of `forge/test/mlir/test_quasar_sim.py`.
