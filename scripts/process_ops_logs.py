    ...
    for device in host_ops_by_device:
        ...
        for device_op, device_op_time in zip(host_ops_by_device[device], device_ops_time):
            if device_op_time is None:
                continue
            # Verify match (warn instead of assert to handle partial buffer overflow data)
            if len(device_op_time["timeseries"]) > 0:
                time_id = device_op_time["timeseries"][0][0]
                if "run_host_id" in time_id:
                    if time_id["run_host_id"] != device_op["global_call_count"]:
                        logger.warning(
                            f"Op ID mismatch: device run_host_id={time_id['run_host_id']} "
                            f"vs host global_call_count={device_op['global_call_count']}; skipping enrichment for this op"
                        )
                        continue
...
def append_device_data(
    ...
    if use_perf_csv:
        ...
        if device_log.is_file():
            logger.info(
                "Running supplemental legacy device-log analysis for per-core timing data."
            )
            try:
                ...
                # Merge per-core device_time entries from legacy path into primary ops
                for device_id, device_ops in host_ops_by_device.items():
                    supp_ops = host_ops_supplemental.get(device_id, [])
                    supp_by_gcc = {op.get("global_call_count"): op for op in supp_ops}
                    for op in device_ops:
                        gcc = op.get("global_call_count")
                        supp = supp_by_gcc.get(gcc)
                        if supp and "device_time" in supp:
                            if "device_time" not in op:
                                op["device_time"] = {}
                            for analysis_key, analysis_data in supp["device_time"].items():
                                if analysis_key not in op["device_time"]:
                                    op["device_time"][analysis_key] = analysis_data
            except Exception as _supp_err:
                logger.warning(
                    f"Supplemental legacy device-log analysis failed (per-core data unavailable): {_supp_err}"
                )
    else:
    ...
    if os.path.isfile(deviceTimesLog):
        ...
        if export_csv:
            with open(allOpsCSVPath, "w") as allOpsCSV:
                ...
                # Include any extra fields produced by analysis (e.g. TRACE KERNEL/FW
                # DURATION fields from --device-trace-profiler) not in the static header.
...
def reconstructNocTracesFromCSV(logFolder: Path) -> bool:
    """Reconstructs per-op NOC trace JSON files from profile_log_device.csv.

    When --dispatch-cores and --noc-traces are used together, the C++ profiler's
    coalesceFabricEvents() hits TT_FATAL on ERISC markers (which carry invalid
    NOC-event type bytes in the dispatch context). The resulting std::runtime_error
    is silently swallowed by the thread pool, so dumpJsonNocTraces() never runs and
    only the ERISC-only noc_trace_dev0_ID0.json is written.

    profile_log_device.csv is always written before the NOC-JSON step, so it
    contains all 2M+ tensix NOC-event rows with correct run_host_id values.
    This function decodes those entries and writes the missing per-op JSON files
    so the existing tt-npe analysis pipeline can proceed unchanged.

    Returns True if files were (re)constructed, False on any failure.
    """
    import struct as _struct
    import numpy as np

    csv_path = logFolder / PROFILER_DEVICE_SIDE_LOG
    if not csv_path.exists():
        return False

    # Skip if per-op files already exist (any file whose name is NOT *_ID0.json)
    existing = list(logFolder.glob("noc_trace_dev*_ID*.json"))
    if any("_ID0.json" not in f.name for f in existing):
    ...
    NOC_EVENT_NAMES = {
        0: "UNDEF", 1: "READ", 2: "READ_SET_STATE", 3: "READ_SET_TRID",
        4: "READ_WITH_STATE", 5: "READ_WITH_STATE_AND_TRID", 6: "READ_BARRIER_START",
        7: "READ_BARRIER_END", 8: "READ_BARRIER_WITH_TRID", 9: "READ_DRAM_SHARDED_SET_STATE",
        10: "READ_DRAM_SHARDED_WITH_STATE", 11: "WRITE_", 12: "WRITE_SET_TRID",
        13: "WRITE_WITH_TRID", 14: "WRITE_INLINE", 15: "WRITE_MULTICAST",
        16: "WRITE_SET_STATE", 17: "WRITE_WITH_STATE", 18: "WRITE_WITH_TRID_SET_STATE",
        19: "WRITE_WITH_TRID_WITH_STATE", 20: "WRITE_BARRIER_START", 21: "WRITE_BARRIER_END",
        22: "WRITE_BARRIER_WITH_TRID", 23: "WRITE_FLUSH", 24: "WRITE_FLUSH_WITH_TRID",
        25: "FULL_BARRIER", 26: "ATOMIC_BARRIER", 27: "SEMAPHORE_INC", 28: "SEMAPHORE_WAIT",
        29: "SEMAPHORE_SET", 30: "SEMAPHORE_SET_REMOTE", 31: "SEMAPHORE_SET_MULTICAST",
        32: "SEMAPHORE_INC_MULTICAST",
    }
    NOC_TYPE_NAMES = {0: "UNDEF", 1: "NOC_0", 2: "NOC_1"}

    # Wormhole B0 (N150/N300): NOC grid is 10 x 12 (x=[0-9], y=[0-11])
    # NOC_1 virtual → NOC_0 physical: phys_x = max_x - virt_x, phys_y = max_y - virt_y
    NOC1_MAX_X = 9
    NOC1_MAX_Y = 11

    EXCLUDED_ZONE_NAMES = {"SYNC-ZONE-SENDER", "SYNC-ZONE-RECEIVER", "PROFILER-NOC-QUICK-SEND"}
    KERNEL_PROCS = {"BRISC", "NCRISC"}

    def _include_zone(zone_name: str, proc: str) -> bool:
        if not zone_name or zone_name in EXCLUDED_ZONE_NAMES:
            return False
        if zone_name.endswith("-FW"):
            return False
        if zone_name.endswith("-KERNEL") and proc not in KERNEL_PROCS:
            return False
        return True
    ...
    # Normalise column names (strip any residual whitespace)
    df.columns = [c.strip() for c in df.columns]

    # Drop ERISC rows and rows without a run_host_id
    df = df[df["RISC processor type"] != "ERISC"]
    run_host_id_col = pd.to_numeric(df["run host ID"], errors="coerce").fillna(0).astype(np.int64)
    df = df[run_host_id_col != 0].copy()
    # Use "rhi" (not underscore-prefixed) so itertuples can access it as a named field
    df["rhi"] = run_host_id_col[df.index]

    # ---- NOC event rows (timer_id == 12345 and type == TS_DATA) ----
    timer_col = pd.to_numeric(df["timer_id"], errors="coerce").fillna(0).astype(np.int64)
    noc_mask = (timer_col == NOC_TRACING_STATIC_ID) & (df["type"] == "TS_DATA")
    noc_df = df[noc_mask].copy()

    # Decode the 8-byte data field using bit-manipulation on int64 values.
    # The struct layout (little-endian) of LocalNocEvent:
    #   byte 0 : noc_xfer_type (uint8)
    #   byte 1 : dst_x  (int8, signed)
    #   byte 2 : dst_y  (int8, signed)
    #   byte 3 : mcast_end_dst_x (int8, signed)
    #   byte 4 : mcast_end_dst_y (int8, signed)
    #   byte 5 : lower 4 bits = noc_type, upper 4 bits = noc_vc (both signed 4-bit)
    #   byte 6 : payload_chunks (uint8)
    #   byte 7 : bit 0 = posted, bits 1-7 = reserved
    data_i64 = pd.to_numeric(noc_df["data"], errors="coerce").fillna(0).astype(np.int64).values

    def _to_signed8(arr):
        return np.where(arr > 127, arr - 256, arr).astype(np.int64)

    def _to_signed4(arr):
        return np.where(arr > 7, arr - 16, arr).astype(np.int64)

    noc_xfer_types = data_i64 & 0xFF
    dst_xs  = _to_signed8((data_i64 >> 8)  & 0xFF)
    dst_ys  = _to_signed8((data_i64 >> 16) & 0xFF)
    mcast_end_xs = _to_signed8((data_i64 >> 24) & 0xFF)
    mcast_end_ys = _to_signed8((data_i64 >> 32) & 0xFF)
    noc_type_raws = (data_i64 >> 40) & 0xF
    noc_vc_raws   = (data_i64 >> 44) & 0xF
    noc_vcs       = _to_signed4(noc_vc_raws)
    payload_chunks_arr = (data_i64 >> 48) & 0xFF
    num_bytes_arr = payload_chunks_arr * PAYLOAD_CHUNK_SIZE

    # Translate NOC addresses to NOC_0 physical coordinates (Wormhole B0).
    # Three cases:
    #   1. Translated (x>=18 or y>=16): fixed TRANSLATED→NOC0 table (same for NOC_0 and NOC_1)
    #      x: 18→1,19→2,20→3,21→4, 22→6,23→7,24→8,25→9  (skip x=5, DRAM column)
    #      y: 18→1,19→2,20→3,21→4,22→5, 23→7,24→8, 25→10 (skip y=6,9, non-tensix rows)
    #   2. Non-translated NOC_1 (noc_type=2): phys = (MAX_X − virt, MAX_Y − virt)
    #   3. Non-translated NOC_0 (noc_type=1): already physical (no change)
    ...
    # Build NOC-event entries grouped by run_host_id
    events_by_op: Dict[int, List[dict]] = defaultdict(list)

    noc_core_xs   = noc_df["core_x"].values
    noc_core_ys   = noc_df["core_y"].values
    noc_procs     = noc_df["RISC processor type"].values
    noc_pcie      = noc_df["PCIe slot"].values
    noc_times     = pd.to_numeric(noc_df["time[cycles since reset]"], errors="coerce").fillna(0).astype(np.int64).values
    noc_run_ids   = noc_df["rhi"].values

    for i in range(len(noc_run_ids)):
        xfer_type = int(noc_xfer_types[i])
        # Skip invalid / unsupported / fabric event types (types 33-41 need coalescing)
        if xfer_type == 0 or xfer_type >= 33:
            continue

        run_id = int(noc_run_ids[i])
        noc_name = NOC_TYPE_NAMES.get(int(noc_type_raws[i]), "UNDEF")
        type_name = NOC_EVENT_NAMES.get(xfer_type, f"UNKNOWN_{xfer_type}")

        entry: dict = {
            "run_host_id": run_id,
            "op_name": "",
            "proc": str(noc_procs[i]),
            "noc": noc_name,
            "vc": int(noc_vcs[i]),
            "src_device_id": int(noc_pcie[i]),
            "sx": int(noc_core_xs[i]),
            "sy": int(noc_core_ys[i]),
            "num_bytes": int(num_bytes_arr[i]),
            "type": type_name,
            "timestamp": int(noc_times[i]),
        }

        dx, dy = int(dst_xs[i]), int(dst_ys[i])
        if dx == -1 or dy == -1 or xfer_type in (READ_WITH_STATE, WRITE_WITH_STATE):
            pass  # no destination coordinates
        elif xfer_type == WRITE_MULTICAST:
            entry["mcast_start_x"] = int(phys_dst_xs[i])
            entry["mcast_start_y"] = int(phys_dst_ys[i])
            entry["mcast_end_x"]   = int(phys_mcast_end_xs[i])
            entry["mcast_end_y"]   = int(phys_mcast_end_ys[i])
        else:
            entry["dx"] = int(phys_dst_xs[i])
            entry["dy"] = int(phys_dst_ys[i])

        events_by_op[run_id].append(entry)

    # ---- Zone rows (ZONE_START / ZONE_END) ----
    zone_mask = df["type"].isin(["ZONE_START", "ZONE_END"])
    zone_df = df[zone_mask].copy()
    if not zone_df.empty:
        # Vectorised zone filter: strip and classify zone names
        zn_series   = zone_df["zone name"].str.strip()
        proc_series = zone_df["RISC processor type"].str.strip()
        is_excluded          = zn_series.isin(EXCLUDED_ZONE_NAMES)
        is_fw                = zn_series.str.endswith("-FW")
        is_kernel_non_brisck = zn_series.str.endswith("-KERNEL") & ~proc_series.isin(KERNEL_PROCS)
        keep_mask = ~(is_excluded | is_fw | is_kernel_non_brisck) & zn_series.ne("")
        zone_df = zone_df[keep_mask].copy()
        # Attach stripped helpers with safe (non-underscore-prefixed) names
        zone_df["zn"]  = zn_series[zone_df.index]
        zone_df["prc"] = proc_series[zone_df.index]

        for row in zone_df.itertuples(index=False):
            run_id = int(row.rhi)
            if run_id == 0:
                continue
            events_by_op[run_id].append({
                "run_host_id": run_id,
                "op_name": "",
                "proc": str(row.prc),
                "src_device_id": int(row[0]),
                "sx": int(row[1]),
                "sy": int(row[2]),
                "timestamp": int(row[5]) if str(row[5]) != "" else 0,
                "zone": str(row.zn),
                "zone_phase": str(row[11]).strip(),
            })

    if not events_by_op:
        logger.warning("No tensix NOC events found in CSV; cannot reconstruct NOC trace files.")
        return False
...
def analyzeNoCTraces(logFolder: Path):
    ...
    try:
        ...
        logger.info("[noc]   quiet=False  (verbose so we can see per-op progress)")
        _t1 = _time.time()
        result = analyze_noc_traces_in_dir(
            noc_trace_dir=logFolder,
            emit_viz_timeline_files=False,
            quiet=False,
