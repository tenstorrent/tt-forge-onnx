...
# ---------------------------------------------------------------------------
# Install patched post-processors into the tt-metal tracy package directory.
# scripts/process_ops_logs.py and scripts/process_device_log.py contain bug
# fixes that are not yet upstream. They are copied (not symlinked) so they
# survive tt-mlir rebuilds that overwrite the install tree.
# ---------------------------------------------------------------------------
TRACY_PKG_DIR="${TRACY_PY_MODULE}/tracy"
if [[ -d "${TRACY_PKG_DIR}" ]]; then
    cp "${REPO_ROOT}/scripts/process_ops_logs.py"  "${TRACY_PKG_DIR}/process_ops_logs.py"
    cp "${REPO_ROOT}/scripts/process_device_log.py" "${TRACY_PKG_DIR}/process_device_log.py"
    echo "  (patched process_ops_logs.py + process_device_log.py installed into ${TRACY_PKG_DIR})"
else
    echo "  WARNING: Tracy package dir not found at ${TRACY_PKG_DIR} — patched post-processors not installed." >&2
fi
...
# ---------------------------------------------------------------------------
# Incompatibility guard: --mid-run-dump + --dispatch-cores causes a hard crash.
# ---------------------------------------------------------------------------
