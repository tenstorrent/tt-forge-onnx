...
def import_device_profile_log(logPath):
    ...
    # Assign as Python list to bypass ArrowDtype string-column rejection in pandas ≥ 2.x.
    for col_idx in [8, 9]:
        int_vals = pd.to_numeric(df.iloc[:, col_idx], errors="coerce").fillna(-1).astype(int).to_list()
        df[df.columns[col_idx]] = int_vals
