...
@pytest.mark.parametrize(
    "optimizer_enabled",
    [True, False],
    ids=["opt_enabled", "opt_disabled"],
)
def test_conv7x7_relu(optimizer_enabled):
    ...
    compiler_cfg = CompilerConfig(mlir_config=MLIRConfig().set_enable_optimizer(optimizer_enabled))
...
@pytest.mark.parametrize(
    "prepare_conv2d_weights_and_bias_enabled",
    [True, False],
    ids=["prepare_conv2d_weights_and_bias_enabled", "prepare_conv2d_weights_and_bias_disabled"],
)
def test_conv7x7_relu_prepare_conv2d_weights_and_bias(prepare_conv2d_weights_and_bias_enabled):
    ...
    if prepare_conv2d_weights_and_bias_enabled:
        mlir_config = mlir_config.set_custom_config("enable-prepare-conv2d-weights-and-bias=True")
    else:
        mlir_config = mlir_config.set_custom_config("enable-prepare-conv2d-weights-and-bias=False")
