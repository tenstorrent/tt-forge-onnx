...
print_onnx_model_info   – run ONNX shape inference and print a structured
                          summary of the model (inputs, outputs, op types,
                          and per-node shapes).
...
def run_onnx_comparison(
    ...
    Returns:
        ...
        keys ``"errors"``, ``"matches"``, ``"tir_outputs"``, and ``"onnx_outputs"``.
    """
...
# ---------------------------------------------------------------------------
# ONNX model info with shape inference
# ---------------------------------------------------------------------------


def print_onnx_model_info(
    onnx_model: "onnx.ModelProto",
    title: str = "ONNX Model (with Shape Inference)",
    run_shape_inference: bool = True,
) -> "onnx.ModelProto":
    """
    Run ONNX shape inference then print a structured model summary.

    Delegates to
    :func:`forge.transpiler.frontends.onnx.utils.onnx_printer.print_onnx_model_with_shapes`
    which renders inputs, outputs, initializers, and per-node tensor
    shapes/dtypes after shape inference has been applied.

    Args:
        onnx_model: Loaded ``onnx.ModelProto``.
        title: Section title forwarded to the printer.
        run_shape_inference: If ``True`` (default), call
            ``onnx.shape_inference.infer_shapes`` before printing so that
            intermediate tensor shapes are propagated through the graph.

    Returns:
        The (possibly shape-inferred) ``onnx.ModelProto``.
    """
    from onnx import shape_inference
    from forge.transpiler.frontends.onnx.utils.onnx_printer import print_onnx_model_with_shapes

    if run_shape_inference:
        onnx_model = shape_inference.infer_shapes(onnx_model)
        print("[OK] ONNX shape inference complete")

    print_onnx_model_with_shapes(onnx_model, title=title)
    return onnx_model
