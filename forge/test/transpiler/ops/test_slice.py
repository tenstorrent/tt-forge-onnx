...
def _create_slice_model_v10_plus(
    ...
    # ONNX Slice inputs are positional: [data, starts, ends, axes, steps].
    # If steps is given but axes is None, we must explicitly provide default axes
    # so that steps is placed at input index 4 (not misinterpreted as axes at index 3).
...
class TestSlice:
    ...
    def test_slice_negative_steps(self, opset_version, data_shape, starts, ends, steps, axes, dtype):
        ...
        try:
            ...
            assert len(comparison["errors"]) == 0, f"Comparison errors: {comparison['errors']}"
            assert all(comparison["matches"].values()), f"Output mismatch: {comparison}"
        except (ConversionError, ValueError, NotImplementedError, AssertionError) as e:
            # Negative steps are not fully supported; skip instead of failing
