...
class TestConstantOfShape:
    ...
    def test_constantofshape_basic(self, opset_version, shape, dtype):
        ...
        assert "Full" in node_types, f"Expected FullNode, got {node_types}"

        # Verify FullNode attributes
        full_nodes = [n for n in tir_graph.nodes if n.op_type == "Full"]
        assert len(full_nodes) == 1, f"Expected 1 FullNode, got {len(full_nodes)}"
        full_node = full_nodes[0]
        assert full_node.attrs["shape"] == shape, f"Shape mismatch: {full_node.attrs['shape']} != {shape}"
        assert full_node.attrs["fill_value"] == 0.0, f"Expected fill_value=0.0, got {full_node.attrs['fill_value']}"
    ...
    def test_constantofshape_custom_value(self, opset_version, shape, value, value_dtype, output_dtype):
        ...
        # Verify FullNode attributes
        full_node = full_nodes[0]
        assert full_node.attrs["shape"] == shape, f"Shape mismatch"
        expected_value = float(value.item())
        assert (
            abs(full_node.attrs["fill_value"] - expected_value) < 1e-6
        ), f"Fill value mismatch: {full_node.attrs['fill_value']} != {expected_value}"
    ...
    def test_constantofshape_scalar(self, opset_version):
        ...
        assert full_nodes[0].attrs["shape"] == ()
    ...
    def test_constantofshape_zero_sized(self, opset_version, shape):
        ...
        full_nodes = [n for n in tir_graph.nodes if n.op_type == "Full"]
        assert len(full_nodes) == 1
        assert full_nodes[0].attrs["shape"] == shape
