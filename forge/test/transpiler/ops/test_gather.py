...
class TestGather:
    ...
    def test_gather_basic(self, opset_version, data_shape, indices_shape, axis, dtype):
        ...
        # Create random data with the correct dtype
        np.random.seed(42)
        np_dtype = _ONNX_TO_NUMPY_DTYPE.get(dtype, np.float32)
        if np.issubdtype(np_dtype, np.integer):
            data = np.random.randint(0, 100, size=data_shape).astype(np_dtype)
        else:
        ...
        # For axis=0 with 2D data → EmbeddingNode (torch.nn.functional.embedding requires 2D weight)
        # For axis=0 with >2D data → IndexSelectNode (converter falls through to AdvIndex path)
        # For axis != 0 → IndexSelectNode
        if axis == 0 and len(data_shape) == 2:
            assert "Embedding" in node_types, f"Expected EmbeddingNode for axis=0 with 2D data, got {node_types}"
        else:
            assert (
                "IndexSelect" in node_types
            ), f"Expected IndexSelectNode for axis={axis} (data_rank={len(data_shape)}), got {node_types}"
    ...
    def test_gather_out_of_range_indices(self, opset_version, data_shape, indices_shape, axis, out_of_range_indices):
        ...
        Note: ONNX Runtime rejects out-of-range indices as an error, so we only verify
        that the TIR graph has a ClipNode for clamping and that TIR execution succeeds.
        """
        # Create random data
        np.random.seed(42)
        data = np.random.randn(*data_shape).astype(np.float32)
        ...
        # Verify TIR executes without error (clamping should prevent out-of-bounds access)
        # We cannot compare with ONNX Runtime here because ORT rejects out-of-range indices
        import torch

        input_dict = {
            "input_0": torch.from_numpy(data),
            "input_1": torch.from_numpy(indices),
        }
        try:
            tir_outputs = tir_graph.run(input_dict)
            assert tir_outputs, "TIR graph produced no outputs"
        except Exception as e:
            raise AssertionError(f"TIR execution failed with out-of-range indices: {e}") from e
