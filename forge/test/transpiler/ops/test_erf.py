...
class TestErf:
    ...
    def test_erf_basic(self, opset_version, input_shape, dtype):
        ...
        assert (
            erf_node.input_names[0] == "input_0"
        ), f"ErfNode input should be 'input_0', got {erf_node.input_names[0]}"
        # Check forge_op_name is set correctly
        assert (
            erf_node.forge_op_name == "Erf"
        ), f"ErfNode forge_op_name should be 'Erf', got {erf_node.forge_op_name}"
