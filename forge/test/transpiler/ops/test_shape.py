...
class TestShape:
    ...
    def test_shape_basic(self, opset_version, input_shape, input_dtype):
        ...
        # The constant should be stored in tir_graph.constants
        output_name = onnx_model.graph.output[0].name

        # Check that constant exists
        assert output_name in tir_graph.constants, f"Shape constant '{output_name}' not found in tir_graph.constants"

        # Get the constant value
        shape_constant = tir_graph.constants[output_name]
    ...
    def test_shape_with_start(self, opset_version, input_shape, start, expected_output):
        ...
        # Verify constant
        output_name = onnx_model.graph.output[0].name
        assert output_name in tir_graph.constants
    ...
    def test_shape_constant_reusable(self, opset_version):
        ...
        # Verify constant can be accessed
        shape_constant = tir_graph.constants[output_name]
        assert isinstance(shape_constant, torch.Tensor)
