...
{
    // Serialize optional<tt::MathFidelity> as nullable string
    const auto fidelity_val = [&]() -> nlohmann::json
    {
        if (!p.compute_cfg_math_fidelity.has_value()) return nullptr;
        return to_pipeline_string(*p.compute_cfg_math_fidelity);
    }();

    // Serialize optional<tt::DataFormat> (weight dtype) as nullable string
    const auto weight_dtype_val = [&]() -> nlohmann::json
    {
        if (!p.experimental_weight_dtype.has_value()) return nullptr;
...
void from_json(const ::nlohmann::json& j, MLIRConfig& p)
{
    ...
    // Compute kernel configuration — stored as nullable pipeline string
    {
        const auto& jf = j.at("compute_cfg_math_fidelity");
        p.compute_cfg_math_fidelity =
            jf.is_null() ? std::nullopt
                         : std::make_optional(math_fidelity_from_string(jf.get<std::string>()));
    }
    j.at("compute_cfg_fp32_dest_acc_en").get_to(p.compute_cfg_fp32_dest_acc_en);
    // Data type / quantization — stored as nullable pipeline string
    {
        const auto& jd = j.at("experimental_weight_dtype");
        p.experimental_weight_dtype =
            jd.is_null() ? std::nullopt
                         : std::make_optional(weight_dtype_from_string(jd.get<std::string>()));
    }
    // Graph transformation passes
...
std::string config_to_pipeline_options(const std::optional<MLIRConfig>& mlir_config)
{
    ...
    if (mlir_config->memory_layout_analysis_policy.has_value())
        options << " memory-layout-analysis-policy="
                << to_pipeline_string(*mlir_config->memory_layout_analysis_policy);
    ...
    if (mlir_config->compute_cfg_math_fidelity.has_value())
        options << " compute-cfg-math-fidelity="
                << to_pipeline_string(*mlir_config->compute_cfg_math_fidelity);
    ...
    if (mlir_config->enable_implicit_broadcast_folding.has_value())
        options << " enable-implicit-broadcast-folding-pass="
                << *mlir_config->enable_implicit_broadcast_folding;
    ...
    if (mlir_config->enable_decomposition_workaround.has_value())
        options << " enable-decomposition-workaround-pass="
                << *mlir_config->enable_decomposition_workaround;
    ...
    if (mlir_config->ttnn_perf_metrics_output_file.has_value() &&
        !mlir_config->ttnn_perf_metrics_output_file->empty())
        options << " ttnn-perf-metrics-output-file="
                << *mlir_config->ttnn_perf_metrics_output_file;
    if (mlir_config->enable_ttnn_perf_metrics_verbose.has_value())
        options << " ttnn-perf-metrics-verbose-output-enabled="
                << *mlir_config->enable_ttnn_perf_metrics_verbose;

    // -----------------------------------------------------------------------
    // Custom configuration passthrough (appended last — takes precedence)
    // -----------------------------------------------------------------------
