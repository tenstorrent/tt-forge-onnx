...
// tt::DataFormat and tt::MathFidelity are the canonical forge types reused
// here so Python users work with the same enums everywhere in the stack.
...
// TTIRToTTNNDevicePipelineOptions (TTNNPipelines.h §OptionNames::
// memoryLayoutAnalysisPolicy).
//
...
//
// See TTNNPipelineOptionsReference.md §2.3 for worked examples.
// ============================================================================
enum class MemoryLayoutAnalysisPolicy
{
    ...
    /// Often faster for CNN-dominated models (e.g. ResNet).
    GreedyL1Interleaved,
...
{
    if (s == "DFSharding")          return MemoryLayoutAnalysisPolicy::DFSharding;
    if (s == "GreedyL1Interleaved") return MemoryLayoutAnalysisPolicy::GreedyL1Interleaved;
    if (s == "BFInterleaved")       return MemoryLayoutAnalysisPolicy::BFInterleaved;
...
// ============================================================================
// Pipeline-string helpers for tt::MathFidelity
//
// tt::MathFidelity (lower_to_forge/common.hpp) is the canonical forge enum
// for arithmetic precision.  These helpers convert it to / from the string
// expected by the `compute-cfg-math-fidelity` pipeline option
// (TTNNPipelines.h, MathFidelityParser.h).
//
// Mapping (pipeline default is hifi4):
//   MathFidelity::LoFi    → "lofi"      — fastest, least precise
//   MathFidelity::HiFi2   → "hifi2"     — good accuracy/speed trade-off
//   MathFidelity::HiFi3   → "hifi3"     — high accuracy
//   MathFidelity::HiFi4   → "hifi4"     — highest accuracy (pipeline default)
//   MathFidelity::Invalid → "undefined" — let the backend select fidelity
//
// Concrete impact: on ResNet-50 matmul blocks, switching from hifi4 → hifi2
// can reduce per-op latency by 15–30 % with negligible top-5 accuracy loss
// for FP16 models.
//
// See TTNNPipelineOptionsReference.md §2.6 for full details.
// ============================================================================
...
{
    if (s == "lofi")      return tt::MathFidelity::LoFi;
    if (s == "hifi2")     return tt::MathFidelity::HiFi2;
    if (s == "hifi3")     return tt::MathFidelity::HiFi3;
    if (s == "hifi4")     return tt::MathFidelity::HiFi4;
    if (s == "undefined") return tt::MathFidelity::Invalid;
    throw std::invalid_argument("Unknown MathFidelity pipeline string: " + s);
}

// ============================================================================
// Pipeline-string helpers for tt::DataFormat (weight dtype context)
//
// The `experimental-weight-dtype` pipeline option (TTNNPipelines.h,
// WeightDtypeParser.h) converts weight tensors in matmul / linear ops to a
// reduced-precision block-floating-point format, reducing DRAM bandwidth for
// weight-heavy models.
//
// The forge stack already carries tt::DataFormat from common.hpp, so we reuse
// its Bfp8_b / Bfp4_b values rather than introducing a redundant enum.
//
// Valid values (pass std::nullopt for "none" — no conversion):
//   DataFormat::Bfp8_b → "bfp_bf8"   8-bit mantissa BFP
//   DataFormat::Bfp4_b → "bfp_bf4"   4-bit mantissa BFP (most aggressive)
//
// All other DataFormat values are rejected by set_experimental_weight_dtype().
//
// See TTNNPipelineOptionsReference.md §2.7 for full details.
// ============================================================================
...
{
    if (s == "bfp_bf8") return tt::DataFormat::Bfp8_b;
    if (s == "bfp_bf4") return tt::DataFormat::Bfp4_b;
...
//
// Design principles:
//  • All fields are std::optional — unset fields are omitted when building the
//    pipeline option string, so the pipeline retains its built-in defaults.
//  • Strongly-typed enums (MemoryLayoutAnalysisPolicy) and reuse of the
//    canonical tt::MathFidelity / tt::DataFormat types replace the raw
//    strings of the older API, catching invalid values before they reach the
//    MLIR backend.
//  • Every setter validates its argument and throws std::invalid_argument or
//    std::out_of_range on bad input.
//  • Setters return *this for convenient method chaining.
//
// Example:
//   MLIRConfig cfg;
//   cfg.set_optimization_level(1)
//      .set_compute_cfg_math_fidelity(tt::MathFidelity::HiFi2)
//      .set_memory_layout_analysis_policy(MemoryLayoutAnalysisPolicy::DFSharding)
//      .set_experimental_weight_dtype(tt::DataFormat::Bfp8_b);
// ============================================================================
struct MLIRConfig
{
    // -------------------------------------------------------------------------
    ...
    //
    //  0 — All optimizer passes disabled. Fastest compile, slowest runtime.
    ...
    //
    // Individual options below always take precedence over this shorthand.
    // Valid range: [0, 2].
    // -------------------------------------------------------------------------
    ...
    /// Pipeline option: enable-const-eval   default: false
    ...
    /// pass.  See MemoryLayoutAnalysisPolicy enum for valid values.
    ...
    ///   8  — fast compile, reasonable quality (default)
    ///   32 — wider search; recommended for tuning production models
    ///   64 — exhaustive; very long compile, best layout quality
    /// Must be > 0.
    ...
    ///   MathFidelity::HiFi2   — good accuracy/speed trade-off (recommended
    ...
    /// faster at the cost of slightly lower precision on FP16/BF16 models.
    ...
    /// reduced-precision block-floating-point format, reducing DRAM bandwidth for
    /// weight-heavy models.  Uses tt::DataFormat from lower_to_forge/common.hpp.
    ///
    ///   DataFormat::Bfp8_b — BFP BFloat8 (8-bit mantissa)
    ///   DataFormat::Bfp4_b — BFP BFloat4 (4-bit mantissa, most aggressive)
    ...
    /// Cancels pairs of inverse operations (e.g. transpose followed by
    /// transpose), reducing unnecessary data movement.
    ...
    /// Pipeline option: enable-permute-matmul-fusion   default: true
    ...
    /// Pipeline option: enable-dram-space-saving-optimization-pass   default: true
    ...
    /// Pipeline option: enable-remove-dead-values   default: true
    ...
    /// Pipeline option: enable-const-eval-inputs-to-system-memory   default: false
    /// Stores the output of const-eval sub-graphs in system (host) memory
    /// instead of device DRAM.  Useful when device DRAM is the bottleneck.
    ...
    /// Pipeline option: ttnn-perf-metrics-verbose-output-enabled   default: false
    /// Enables verbose mode in the TTNN performance metrics report, including
    /// per-core breakdowns and kernel configuration details.
    ...
    // See TTNNPipelineOptionsReference.md for all available options.
    // -------------------------------------------------------------------------
    ...
    // Optimizer control
    MLIRConfig& set_enable_consteval(bool v)              { enable_consteval = v; return *this; }
    MLIRConfig& set_enable_optimizer(bool v)              { enable_optimizer = v; return *this; }
    MLIRConfig& set_enable_memory_layout_analysis(bool v) { enable_memory_layout_analysis = v; return *this; }
    ...
    MLIRConfig& set_enable_memreconfig(bool v) { enable_memreconfig = v; return *this; }
    ...
    MLIRConfig& set_enable_row_major(bool v) { enable_row_major = v; return *this; }
    ...
    MLIRConfig& set_compute_cfg_fp32_dest_acc_en(bool v) { compute_cfg_fp32_dest_acc_en = v; return *this; }
    ...
    // Graph transformation passes
    MLIRConfig& set_enable_erase_inverse_ops(bool v)          { enable_erase_inverse_ops = v; return *this; }
    MLIRConfig& set_enable_implicit_broadcast_folding(bool v)  { enable_implicit_broadcast_folding = v; return *this; }
    MLIRConfig& set_enable_permute_matmul_fusion(bool v)       { enable_permute_matmul_fusion = v; return *this; }
    MLIRConfig& set_enable_dram_space_saving_optimization(bool v)
    {
        enable_dram_space_saving_optimization = v;
        return *this;
    }
    MLIRConfig& set_enable_remove_dead_values(bool v) { enable_remove_dead_values = v; return *this; }
    ...
    // Fusing passes
    MLIRConfig& set_enable_fusing(bool v)    { enable_fusing = v; return *this; }
    MLIRConfig& set_enable_d2m_fusing(bool v) { enable_d2m_fusing = v; return *this; }
    MLIRConfig& set_fusing_conv2d_with_multiply_pattern(bool v)
    {
    ...
    // Execution options
    MLIRConfig& set_enable_trace(bool v) { enable_trace = v; return *this; }

    // Workaround passes
    MLIRConfig& set_disable_workarounds(bool v)             { disable_workarounds = v; return *this; }
    MLIRConfig& set_enable_layout_workaround(bool v)        { enable_layout_workaround = v; return *this; }
    MLIRConfig& set_enable_decomposition_workaround(bool v) { enable_decomposition_workaround = v; return *this; }

    // Performance metrics and diagnostics
    MLIRConfig& set_enable_ttnn_perf_metrics(bool v)        { enable_ttnn_perf_metrics = v; return *this; }
    MLIRConfig& set_ttnn_perf_metrics_output_file(const std::string& path)
    {
        ttnn_perf_metrics_output_file = path;
        return *this;
    }
    MLIRConfig& set_enable_ttnn_perf_metrics_verbose(bool v) { enable_ttnn_perf_metrics_verbose = v; return *this; }

    // Custom configuration passthrough
    MLIRConfig& set_custom_config(const std::string& config) { custom_config = config; return *this; }
};
