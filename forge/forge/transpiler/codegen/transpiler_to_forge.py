...
def generate_forge_module_from_transpiler(
    ...
    if tir_graph.computed_constants:
        # De-duplicate: keep only the sanitized name (primary key); skip original-name aliases
        # to avoid storing the same tensor twice under two names.
        sanitized_names = set(tir_graph.original_to_sanitized.values())
        unique_computed = {
            name: tensor
            for name, tensor in tir_graph.computed_constants.items()
            if name in sanitized_names or name not in tir_graph.original_to_sanitized.values()
        }
