...
from forge.transpiler.operations.comparison import LogicalNotNode
...
class UnaryOpConverter(OnnxOpConverter):
    ...
    # Operations whose output dtype is always bool regardless of input dtype.
    # For these, UnaryOpConverter overrides the pass-through dtype logic.
    _BOOL_OUTPUT_OPS = {"Not"}
    ...
    def convert(
        ...
        # For unary operations, output shape always matches input shape.
        # Output dtype matches input dtype unless the op always produces bool
        # (e.g. Not: input tensor(bool) → output tensor(bool)).
        ...
        output_onnx_dtype = onnx.TensorProto.BOOL if op_type in cls._BOOL_OUTPUT_OPS else input_tensor.onnx_dtype
        output_tensors[output_name] = TensorInfo(
            name=output_name,
            shape=input_tensor.shape,
            onnx_dtype=output_onnx_dtype,
        )
