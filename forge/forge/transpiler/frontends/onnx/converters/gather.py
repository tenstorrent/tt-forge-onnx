...
class GatherConverter(OnnxOpConverter):
    ...
    2. If axis=0 → EmbeddingNode (uses forge.op.Embedding)
       - Creates preprocessing nodes: Cast (to int64), Where (normalize negatives), Clip (clamp)
       - EmbeddingNode uses ONLY torch.nn.functional.embedding
    ...
    def convert(
        ...
        Returns:
            ...
            - OR multiple nodes: Cast, Where, Clip, EmbeddingNode (if axis=0)
            - OR multiple nodes: Cast, Reshape, Where, Clip, IndexSelectNode (if axis != 0)
        ...
        # 1. Check if indices are constant and form a contiguous range → use Index
        # 2. Check if axis=0 → use Embedding (only works for axis=0)
        # 3. Otherwise → use IndexSelect (maps to AdvIndex, handles any axis)

        # Check if indices is a constant initializer
        indices_is_constant = False
        indices_value = None
        if graph_proto is not None:
            initializer_names = {init.name for init in graph_proto.initializer}
            if indices_name in initializer_names:
                # Get the initializer value
                for init in graph_proto.initializer:
                    if init.name == indices_name:
                        indices_array = numpy_helper.to_array(init)
        ...
        use_index = False
        if indices_is_constant and indices_value is not None:
        ...
        if use_index:
            ...
            # Use Embedding for axis=0 (embedding only supports axis=0)
            logger.trace(
                f"Gather node '{node_name}': Converting to EmbeddingNode " f"(axis=0) with preprocessing nodes"
            )
            ...
            # Use IndexSelect (maps to AdvIndex) for other axes
            logger.trace(
                f"Gather node '{node_name}': Converting to IndexSelectNode " f"(axis={axis}) with preprocessing nodes"
            )

            # Preprocess indices: cast, reshape if needed, normalize negatives, clamp
            current_indices_name = cls._cast_indices_to_int64(
                node_proto, node_name, current_indices_name, indices_tensor, current_outputs, nodes
            )

            current_indices_name = cls._reshape_indices_if_needed(
                node_proto, node_name, current_indices_name, current_outputs, nodes
            )
