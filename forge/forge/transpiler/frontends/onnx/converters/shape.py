...
class CastConverter(OnnxOpConverter):
    ...
    def convert(
        ...
        if opset >= 13 and len(node_proto.input) > 1:
            ...
            _, to_dtype, _ = validate_constant_input(node_proto, input_index=1, graph_proto=graph_proto)
            ...
            if to_dtype is not None:
                try:
                    ...
                    # Normalization failed; keep the original value so downstream
                    # dtype handling can decide how to proceed.
                    logger.debug(
                        f"CastConverter: failed to normalize 'to' dtype '{to_dtype}' to int "
                        f"for node '{node_name}': {exc}"
                    )
                ...
                for input_vi in graph_proto.input:
                    if input_vi.name == data_input_name and input_vi.type.tensor_type.HasField("shape"):
                        ...
                        if len(original_shape) == len(input_info.shape):
                            ...
                            recovered_shape = []
                            for orig_dim, inferred_dim in zip(original_shape, input_info.shape):
