...
class EmbeddingNode(EmbeddingShape, TIRNode):
    ...
    def eval(self, input_tensors: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        **ONLY uses torch.nn.functional.embedding** - no other operations.
        ...
        by separate converter nodes before this operation. For data with ndim > 2,
        the converter routes to IndexSelectNode instead of EmbeddingNode.

        Args:
            input_tensors: Dictionary mapping input names to tensors
            - input_tensors[self.input_names[0]]: embedding_table (data), must be 2D
...
class IndexSelectNode(IndexSelectShape, TIRNode):
    ...
    Constraint: index must be 1D (enforced by the GatherConverter which flattens
    multi-dim indices before this node and adds a ReshapeNode after for the output).
    """
    ...
    def eval(self, input_tensors: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        **ONLY uses torch.index_select** - no other operations.
        ...
        Note: Index normalization, type casting, flattening to 1D, and output reshaping
        for multi-dimensional indices are all handled by separate converter nodes before
